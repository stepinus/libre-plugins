...
WASM 技能在 LibreFang 进程内的 `WasmSandbox` 中运行：受能力门控，并对每次调用施加燃料、线性内存和墙钟时间限制。
它们非常适合安全敏感的工具，因为除已授予的能力外无法触及任何东西。

用 Rust 编写时，最简单的方式是 `librefang-skill` SDK，它把底层 guest ABI 收敛到一个宏后面。

### 构建 WASM 技能（Rust）

1. 创建一个依赖 `librefang-skill` 的 `cdylib` crate：
...
[profile.release]
panic = "abort"   # guest 不做栈展开；同时减小模块体积
```

2. 编写 handler 并用 `skill!` 宏注册：
...
handler 收到的是所有 runtime 都用的同一份 `{tool, input, config}` envelope。
返回 `Err` 会以 `{"error": ...}` 的形式呈现给 agent，与子进程 runtime 非零退出的语义一致。

3. 以 `wasm32-unknown-unknown` 为目标构建，并把产物复制到技能根目录：
...
<Note>
  目标是 `wasm32-unknown-unknown`，**不是** `wasm32-wasi`。
  沙箱只提供 `librefang` 这组 host import（`host_call`、`host_log`）；WASI 模块会导入 `wasi_snapshot_preview1`，从而实例化失败。
  把产物复制出 `target/`，是因为技能打包器会排除构建目录。
</Note>
...
`librefang skill create`（选 `wasm` runtime）会生成以上全部脚手架，`librefang skill test <dir>` 会通过真实沙箱在本地运行 WASM 技能。

### Host 调用

WASM 技能通过 `librefang_skill::host` 访问 host 功能。
每个方法都需要 `[requirements] capabilities` 中对应的能力，其中若干会消耗燃料（denial-of-wallet 防护）：

| 函数 | 能力 | 燃料 |
| --- | --- | --- |
| `time_now`、`kv_get`、`kv_set` | — | 免费 |
| `env_read` | `EnvRead(<glob>)` | 免费 |
| `fs_read`、`fs_list` | `FileRead(<glob>)` | 免费 |
| `fs_write` | `FileWrite(<glob>)` | 免费 |
| `net_fetch` | `NetConnect(<host:port>)` | 计费 |
| `shell_exec` | `ShellExec(<glob>)` | 计费 |
| `agent_send` | `AgentMessage(<glob>)` | 计费 |
| `agent_spawn` | `AgentSpawn` | 计费 |

无法解析的能力字符串会被丢弃（默认拒绝），所以拼写错误意味着"未授予"，而不是悄悄放大权限。

### 沙箱限制

WASM 沙箱对每次调用强制执行：

- **燃料限制** —— CPU 指令预算，同时限制 host 调用的扇出（防止无限循环和 denial-of-wallet）。默认 1,000,000。
- **内存限制** —— 线性内存的最大增长。默认 16 MiB。
- **墙钟超时** —— 取自 `[requirements] timeout_secs`，否则为 30 秒。
- **能力** —— 仅授予技能声明（且能解析）的能力，其余一律拒绝。
