...
import { zhNavigation, enNavigation } from "@/components/Navigation";
...
function SearchResult({
	...
	const allNavigation = [...enNavigation, ...zhNavigation];
	const sectionTitle = allNavigation.find(
		(section: { links: Array<{ href: string }> }) =>
			section.links.find(
				(link: { href: string }) => link.href === result.url.split("#")[0],
			),
	)?.title;
